...
export function Navbar({user, authLoading, authError, login, logout}) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault(); //ทำให้ไม่เกิดการ refresh page
    setSubmitting(true);
    ...
    <nav>
      <div className="flex justify-between px-10 items-center w-full bg-teal-500 h-14 border-b-2 border-black gap-x-6 text-2xl text-white ">
        ...
        <div className="flex items-center gap-x-3">
          {authLoading ? (
            <span className="text-base">Checking auth session ( •̀ ω •́ )✧</span>) : user ? (
            <>
              <span className="text-base">Logged in as <span>{user.username}</span></span>
              <button onClick={logout} className="cursor-pointer bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded-xl text-base">Logout</button>
            </>
            ) : (
